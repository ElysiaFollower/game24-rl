...
- 当前本地状态：fixed single-path trace 决策、报告补充和实验脚本已落盘；短实验和高 LR probe 已在 AutoDL 完成。
- 最新已推送代码 commit：`372e17a`。当前 fixed-trace 实验脚本和文档更新尚未提交，已用 `rsync` 同步到 AutoDL。
...
Fixed single-path trace experiment 和 higher-LR probe 已完成。当前任务是分析为什么 fixed trace 未带来预期提升，并决定下一组最小实验。
...
读取两个 summary 和 raw outputs，做 fixed-trace failure case study；随后决定是 full finetune、扩充每题多路径、还是回到 baseline format/curriculum 差异。
