...
Baseline-format v2 full finetune 已形成当前最强备案结果
`51/136 = 37.50%`。当前任务是新开分支，对照 reference baseline 继续定位
剩余差距并提高准确率。
...
- baseline-format v2 full finetune 达到 `37.50%`，说明更接近 baseline 的数据
  和 full finetune 有正向收益；但 `format_rate=38.97%` 表明长 search/rollback
  trace 与当前输出契约/生成设置仍有明显错配。
...
在 `dev/baseline-accuracy-improvement` 上读取
`outputs/experiments/baseline_format_v2_full/eval/final/validation-raw-outputs.jsonl`
和 reference baseline 实现，先做 failure case study，再决定是修 prompt/closure、
调 generation 长度、调整数据格式，还是进一步贴近 baseline 训练 recipe。
